/**
 * Local order feed: keeps one authenticated wsjson session to the ToS gateway
 * and re-broadcasts the live order book as JSON over a localhost WebSocket, so
 * desktop UIs (e.g. the GPUI watcher) don't need to speak wsjson or hold the
 * session token.
 *
 *   node --env-file=.env dist/example/orderFeedServer.js [--port 8787]
 *
 * Wire protocol (one JSON object per WebSocket text frame):
 *   { "type": "session", "account": "D-…", "tradingSystem": "PaperMoney" }
 *   { "type": "orders",  "orders": [ …OrderRow… ], "at": 1690000000000 }
 *   { "type": "connection", "state": "connected" | "reconnecting" | "disconnected" | "gaveUp" }
 * A fresh client immediately receives `session`, the latest `orders`, and the
 * current `connection` state.
 */
import { WebSocketServer, WebSocket as WsSocket } from "ws";
import { RealWsJsonClient } from "../client/realWsJsonClient.js";
import {
  BrowserSession,
  captureBrowserSession,
  saveSessionToDotEnv,
  sessionFromEnv,
} from "./browserSession.js";

export type OrderRow = {
  orderId: number;
  symbol: string;
  side: string;
  quantity: number;
  filledQuantity: number;
  price?: number;
  orderType?: string;
  tif?: string;
  status: string;
  active: boolean;
  description: string;
  updatedAt: number;
};

/** Statuses that mean "still live at the exchange". */
const ACTIVE_STATUSES = new Set([
  "WORKING",
  "QUEUED",
  "PENDING",
  "WAIT TRG",
  "WAIT_TRIGGER",
  "OPEN",
]);

type RawOrder = {
  orderId?: number;
  status?: string;
  eventType?: string;
  side?: string;
  quantity?: number;
  filledQuantity?: number;
  price?: number;
  limitPrice?: number;
  orderType?: string;
  tif?: string;
  legs?: { symbol?: string }[];
  rootSymbol?: string;
  descriptionToShare?: string;
  description?: string;
  eventTime?: number;
};

export function toOrderRow(o: RawOrder, now = Date.now()): OrderRow {
  const status = (o.status ?? o.eventType ?? "UNKNOWN").toUpperCase();
  return {
    orderId: o.orderId ?? 0,
    symbol: o.legs?.[0]?.symbol ?? o.rootSymbol ?? "",
    side: o.side ?? "",
    quantity: o.quantity ?? 0,
    filledQuantity: o.filledQuantity ?? 0,
    price: o.price ?? o.limitPrice,
    orderType: o.orderType,
    tif: o.tif,
    status,
    active: ACTIVE_STATUSES.has(status),
    description: o.descriptionToShare ?? o.description ?? "",
    updatedAt: o.eventTime ?? now,
  };
}

export class OrderFeed {
  private readonly rows = new Map<number, OrderRow>();

  /** Merges an order_events payload; returns the full current book. */
  apply(orders: RawOrder[], now = Date.now()): OrderRow[] {
    for (const raw of orders) {
      const row = toOrderRow(raw, now);
      if (!row.orderId) continue;
      const prev = this.rows.get(row.orderId);
      // Later events win; a terminal status must never be overwritten by a
      // stale WORKING snapshot replayed after a reconnect.
      if (prev && !prev.active && row.active) continue;
      this.rows.set(row.orderId, { ...prev, ...row });
    }
    return this.snapshot();
  }

  snapshot(): OrderRow[] {
    return [...this.rows.values()].sort(
      (a, b) =>
        Number(b.active) - Number(a.active) || b.updatedAt - a.updatedAt,
    );
  }
}

async function resolveSession(): Promise<BrowserSession> {
  const cached = sessionFromEnv();
  if (cached) return cached;
  const session = await captureBrowserSession({ tradingSystem: "PaperMoney" });
  saveSessionToDotEnv(session);
  return session;
}

export async function startOrderFeedServer(port = 8787) {
  const session = await resolveSession();
  const feed = new OrderFeed();
  const wss = new WebSocketServer({ host: "127.0.0.1", port });
  const clients = new Set<WsSocket>();
  let connectionState = "connecting";

  const broadcast = (msg: unknown) => {
    const data = JSON.stringify(msg);
    for (const c of clients) {
      if (c.readyState === c.OPEN) c.send(data);
    }
  };

  const client = await RealWsJsonClient.create({
    tradingSystem: session.tradingSystem,
    gatewayUrl: session.gatewayUrl,
    watchdog: {
      onConnectionEvent: (event) => {
        connectionState =
          event.type === "connected" ? "connected" : event.type.toLowerCase();
        broadcast({
          type: "connection",
          state: connectionState,
          detail: event,
        });
        console.log("[feed] connection:", JSON.stringify(event));
      },
    },
  });
  await client.authenticateWithAccessToken({
    accessToken: session.accessToken,
    refreshToken: session.refreshToken ?? "n/a",
  });
  const account =
    session.accountCode ??
    (await client.resolveAccountCode()) ??
    (() => {
      throw new Error("could not resolve an account code");
    })();

  wss.on("connection", (ws) => {
    clients.add(ws);
    ws.send(
      JSON.stringify({
        type: "session",
        account,
        tradingSystem: session.tradingSystem,
      }),
    );
    ws.send(
      JSON.stringify({
        type: "orders",
        orders: feed.snapshot(),
        at: Date.now(),
      }),
    );
    ws.send(JSON.stringify({ type: "connection", state: connectionState }));
    ws.on("close", () => clients.delete(ws));
  });

  console.log(
    `[feed] serving ws://127.0.0.1:${port} — ${session.tradingSystem} account ${account}`,
  );

  void (async () => {
    for await (const ev of client.orderEvents(account)) {
      const orders = feed.apply((ev.body.orders as RawOrder[]) ?? []);
      broadcast({ type: "orders", orders, at: Date.now() });
      const active = orders.filter((o) => o.active).length;
      console.log(`[feed] ${orders.length} orders (${active} active)`);
    }
  })();

  const shutdown = () => {
    console.log("[feed] shutting down");
    client.disconnect();
    wss.close();
    process.exit(0);
  };
  process.on("SIGINT", shutdown);
  process.on("SIGTERM", shutdown);
  return { client, wss, feed };
}

if (process.argv[1]?.endsWith("orderFeedServer.js")) {
  const portArg = process.argv.indexOf("--port");
  const port = portArg > -1 ? Number(process.argv[portArg + 1]) : 8787;
  startOrderFeedServer(port).catch((e) => {
    console.error(e);
    process.exit(1);
  });
}
